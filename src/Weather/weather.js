...
// До вашого додатка з прогнозом погоди можна додати ряд додаткових функцій, які забезпечать кращий користувацький досвід та більше інформації. Ось кілька ідей:
...
//     const weatherItem = document.createElement('li');
//     weatherItem.classList.add('WeatherWrapper'); // Додано клас
//     weatherItem.innerHTML = `
//   <h3 class="DateWrapper">${dateStr} ${timeStr}</h3>
//   <img class="IconStyled" src="${iconUrl}" alt="значок погоди">
//   <p class="DegStyled">Температура: ${tempStr}</p>
//   <p>Вологість: ${humidityStr}</p>
//   <p class="WindStyled">Швидкість вітру: ${windStr}</p>
// `;
...
// initApp();
    ...
    //   <p>Вологість: ${humidityStr}</p>
    //   <p class="WindStyled">Швидкість вітру: ${windStr}</p>
    //   ${numDays === 1 ? `<p>Тиск: ${weather.main.pressure}</p>` : ''}
    //   ${numDays === 1 ? `<p>Видимість: ${weather.main.visibility}</p>` : ''}
    //   ${numDays === 1 ? `<p>Стан неба: ${weather.weather[0].skyStatus}</p>` : ''}
    //   ${numDays === 1 ? `<p>Індекс УФ-випромінювання: ${weather.uvi}</p>` : ''}
    //   ${
    //     numDays === 1
    //       ? `<p>Ймовірність опадів: ${weather.weather[0].precipitation}</p>`
    //       : ''
    //   }
    // `;